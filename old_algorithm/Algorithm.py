from old_algorithm.AnticipatoryNetwork import *
    ...
    def go(self) -> List[Chain]:
        ...
        for chain in self.chains:
                ...
                for source_node in j_m:
                    ...
                    for ftn in self.an.feedbacks:
                        if ftn.target_node.id == target_node and ftn.source_node.id == source_node:
                            recommended_decision = ftn.recommended_decision_ids
                            break
            ...
            for node in self.all_nodes:
                condensed_chains[node.id] = set()
                for chain in summed_chains_lists:
                    if node.id in chain.nodes_order:
                        condensed_chains[node.id].add(chain.decision_order[chain.nodes_order.index(node.id)])
            temp.append(condensed_chains)
